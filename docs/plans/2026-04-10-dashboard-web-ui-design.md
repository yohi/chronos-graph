...
> **作成日**: 2026-04-10
> **更新日**: 2026-04-10 (CQRS / SRP / YAGNI / TDD / Read-Only 再定義レビュー対応 + asyncio.Queue バックプレッシャ仕様修正 + Docker bind/Neo4j Phase 整合性修正 + StorageAdapter DRY/YAGNI 修正 + TDD完全性/DB未初期化フェイルファスト/ログバースト耐性注記 + SQLite IN句チャンク分割注記/統計クエリキャッシュ方針 + SPAフォールバック/WS再接続/API Base URL方針)
> **ブランチ**: `feat/dashboard`
> **ステータス**: Approved (rev. 9)
...
- **完全 Read-Only アーキテクチャ**: Dashboard は**純粋な可視化ツール**として位置づけ、書き込み操作 (DELETE/UPDATE 等) は一切提供しない。これにより以下を保証する:
  - SQLite: `sqlite3.connect("file:...?mode=ro", uri=True)` で接続。`PRAGMA journal_mode=WAL` は MCP 側 (書き込み主体) の初期化時に有効化済みであることを前提とし、dashboard 側では migration を実行しない
...
| `src/context_store/storage/protocols.py` | `GraphAdapter` に `list_edges_for_memories(memory_ids)` と `count_edges()` を追加。`StorageAdapter` は既存メソッド (`get_memory`, `list_by_filter`) をそのまま利用し、**新規メソッドは追加しない** |
| `src/context_store/storage/sqlite.py` | 変更なし (既存の `get_memory`, `list_by_filter` で Dashboard の要件を充足) |
| `src/context_store/storage/postgres.py` | 変更なし (同上) |
| `src/context_store/storage/sqlite_graph.py` | `GraphAdapter` の `list_edges_for_memories`, `count_edges` を実装 |
| `src/context_store/storage/neo4j.py` | `GraphAdapter` の `list_edges_for_memories`, `count_edges` を**両方とも Phase 1 で実装**。`/api/graph/layout` の統合テストが Neo4j 環境でも Green になることを Phase 1 完了条件とする |
...
2. **失敗するユニットテストの作成**: `GraphAdapter` プロトコルの新規メソッド (`list_edges_for_memories`, `count_edges`) に対する期待動作を定義
3. **`GraphAdapter` プロトコル + `sqlite_graph.py` / `neo4j.py` に実装** (テスト Green 化)。`neo4j.py` の `list_edges_for_memories()` も `count_edges()` も **Phase 1 で完全実装** し、統合テストが全バックエンドで Green になることを確認する。TDD プロセスの完全性を担保するため、`NotImplementedError` による先送りは行わない
4. **失敗するユニットテストの作成**: `DashboardService` の集約メソッド (`get_stats_summary`, `get_graph_layout`, `get_project_stats`, `traverse_graph`) に対する期待動作を定義 (mock `StorageAdapter` / `GraphAdapter` 使用)。`get_graph_layout` は既存の `storage.list_by_filter(MemoryFilters)` を利用してデータを取得する集約ロジックをテストする
5. **`DashboardService` を実装** (テスト Green 化)
6. **失敗する統合テストの作成**: 各 REST エンドポイントの正常系・異常系レスポンスを定義 (`httpx.AsyncClient` + mock `DashboardService`)
7. `api_server.py`: FastAPI app + lifespan (`create_storage(settings)` で Read-Only `StorageAdapter` / `GraphAdapter` を初期化、`DashboardService` を組み立て、シャットダウン時に各アダプタの `dispose()`) + CORS + `TrustedHostMiddleware`。uvicorn は `host="0.0.0.0"` でバインド (§2.2 認証方針参照)
8. `schemas.py`: Pydantic レスポンスモデル (`alias_generator=to_camel`, `populate_by_name=True`)
9. `routes/stats.py`: `/api/stats/summary`, `/api/stats/projects` (テスト Green 化)
10. `routes/memories.py`: GET (Read-Only のみ。search は Phase 3 以降) (テスト Green 化)
11. `routes/graph.py`: `/api/graph/layout`, `/api/graph/traverse` (`graph is None` で 503) (テスト Green 化)
12. `routes/system.py`: `/api/system/config` (ホワイトリスト方式) (テスト Green 化)
13. `pyproject.toml` に依存関係 + エントリポイント追加
...
| ファイル | 役割 | 変更の有無 |
|----------|------|-----------|
| `src/context_store/orchestrator.py` | Orchestrator + `create_orchestrator` ファクトリ | **変更なし** (SRP / YAGNI: Dashboard 用メソッドは追加しない) |
| `src/context_store/storage/protocols.py` | StorageAdapter / GraphAdapter プロトコル | `StorageAdapter` は**変更なし** (既存の `get_memory`, `list_by_filter` で充足)。`GraphAdapter` に `list_edges_for_memories`, `count_edges` 追加 |
| `src/context_store/storage/sqlite.py` | SQLiteStorageAdapter | **変更なし** (既存の `get_memory`, `list_by_filter` をそのまま利用) |
| `src/context_store/storage/postgres.py` | PostgresStorageAdapter | **変更なし** (同上) |
| `src/context_store/storage/sqlite_graph.py` | SQLiteGraphAdapter | `list_edges_for_memories`, `count_edges` 実装 |
| `src/context_store/storage/neo4j.py` | Neo4jGraphAdapter | `list_edges_for_memories`, `count_edges` を**両方とも Phase 1 で完全実装** |
| `src/context_store/config.py` | Settings クラス | `dashboard_*` 設定追加 |
