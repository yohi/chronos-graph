...
**排他制御 (Race Condition対策):** 同一コンテンツまたはURLの並行処理による重複登録（TOCTOU脆弱性）を防ぐため、Ingestion Pipeline レベルでコンテンツハッシュやURL等に基づく一時的な非同期ロック（メモリ上のハッシュベースの `asyncio.Lock` 機構、または複数プロセス/Pod環境を考慮したRedisの分散ロック等）を導入し、Deduplicator（検索・重複チェック）から保存完了までの区間を保護してください。さらに、永続化層の最終防衛ラインとして、データベースレベルでの一意制約（`content_hash` 列に対する `UNIQUE` インデックス等）と重複時のエラー/Upsertハンドリングを実装し、マルチプロセス環境でも安全に動作するようにしてください。
...
- 最大トークン制限（`tiktoken` 等を用いた正確なトークン計算を実装）
  ...
  - **近似による過大推定でのガード**: デフォルトエンコーディング（未指定時の `cl100k_base` 等）でのカウントも利用不可能な場合の最終手段として文字数近似を使用する。この際は、**必ず安全側（過大推定）**となる式（例: `ceil(len(text) * safety_margin)` 、safety_margin は 3.0 等）を採用し、`max_tokens` 超過を防ぐためのガードとする。日本語テキストでは1文字が1〜3トークンとなるため、最悪ケースをカバーし厳密な過大推定となるよう3.0以上の係数を採用すること。
