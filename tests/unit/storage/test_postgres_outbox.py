...
@pytest.mark.asyncio
async def test_delete_memory_writes_outbox_when_deleted() -> None:
    from context_store.storage.postgres import PostgresStorageAdapter

    adp = PostgresStorageAdapter.__new__(PostgresStorageAdapter)
    fake_conn = MagicMock()
    fake_conn.fetchrow = AsyncMock(
        return_value={"memory_type": "semantic", "tags": ["t1"], "project": "p1"}
    )
    fake_conn.execute = AsyncMock(return_value="DELETE 1")
    ...
    deleted = await adp.delete_memory("33333333-3333-3333-3333-333333333333")

    assert deleted is True
    outbox.enqueue_sync.assert_awaited_once()
    call = outbox.enqueue_sync.await_args
    assert call.kwargs["event_type"] == "DELETE_MEMORY"
    assert call.kwargs["payload"] == {"memory_type": "semantic", "tags": ["t1"], "project": "p1"}


@pytest.mark.asyncio
async def test_delete_memory_does_not_write_outbox_when_not_found() -> None:
    from context_store.storage.postgres import PostgresStorageAdapter

    adp = PostgresStorageAdapter.__new__(PostgresStorageAdapter)
    fake_conn = MagicMock()
    fake_conn.fetchrow = AsyncMock(return_value=None)
    fake_conn.execute = AsyncMock(return_value="DELETE 0")
    ...
    outbox.enqueue_sync.assert_not_called()
