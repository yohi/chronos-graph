...
- SQLite バックエンドの場合、クリーンアップ処理の最後に肥大化した WAL を安全に縮小するため、非同期タスク内で `PRAGMA wal_checkpoint(PASSIVE)` を実行する。基本は `PASSIVE` を用いるが、**WAL肥大化のフェイルセーフ**として、`PRAGMA wal_checkpoint(PASSIVE)` が継続的に失敗（同一ジョブで連続3回失敗、または10分間で5回以上失敗）し、かつ WAL ファイルの物理サイズが閾値（例: 100MB）を超えた場合にのみ、警告ログを出力した上で `PRAGMA wal_checkpoint(TRUNCATE)` を試行するフォールバックロジックを実装すること。判定ロジックは該当ジョブのステータス管理（カウンタ等）に追加し、実行結果とWALファイルサイズの推移をメトリクスとして収集・可視化する要件を含めること。
