...
- SQLite バックエンドの場合、クリーンアップ処理の最後に肥大化した WAL を安全に縮小するため、非同期タスク内で `PRAGMA wal_checkpoint(PASSIVE)` を実行する。基本は `PASSIVE` を用いるが、**WAL肥大化のフェイルセーフ**として、WALファイルの物理サイズを監視し、閾値（例: 100MB）を超えた場合は警告ログを出力しつつ `PRAGMA wal_checkpoint(TRUNCATE)` を試行するフォールバックロジックを追加すること。さらに、実行結果とWALファイルサイズの推移をメトリクスとして収集・可視化する要件を含めること。
