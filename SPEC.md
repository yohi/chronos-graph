...
5. コネクション確立後、まずレスポンスヘッダーの `Content-Type` が許可リストに含まれることを検証し、その後レスポンスボディを `httpx` のストリーミングリクエスト (`stream`) で受信して、チャンクごとに受信累積サイズを監視する。10MB を超過した時点で直ちに通信を中断 (Abort) すること。これにより巨大ファイルによるプロセス側の OOM (メモリ枯渇) やネットワーク帯域の浪費を防止する。
