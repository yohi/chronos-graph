...
- **WALステータスの永続化スキーマと更新ルール:**
    - 判定用のカウンタやステータスは `lifecycle_state` テーブル等に漠然と保持するのではなく、`lifecycle_state` テーブル（または新規の `lifecycle_wal_state` テーブル）に以下の明確な名前と型のカラムを追加し永続化する：
        ...
        - `wal_failure_window` (JSON): 過去10分間の失敗タイムスタンプの配列（リングバッファ）を保持するスライディングウィンドウ
    - **非同期チェックポイントタスクによる更新ルール:**
        ...
        - スライディングウィンドウの更新：状態更新のたびに `wal_failure_window` から現在時刻より10分以上古いエントリをパージする。このフィールドをSource of Truthとして「10分以内の失敗回数」を計算し、これを用いて「10分間に5回以上の失敗」などの閾値判定を行うこと。
    ...
    - **トランザクションと起動時の復元:**
        ...
        - アプリケーションは起動時にこれらのフィールドを読み込み、プロセス再起動後も失敗カウンタや状態を復元してから、`TRUNCATE` をトリガーするべきかどうかの判定を行うこと。
