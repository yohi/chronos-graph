...
class Settings(BaseSettings):
    ...
    url_allowed_content_types: list[str] = [
...
- `graceful_shutdown()`: `SIGINT`/`SIGTERM` 時に進行中タスクをタイムアウト付き（5秒）で完了待機
...
- クリーンアップ開始時に `cleanup_running=true` + `updated_at` を設定
- クリーンアップ完了時に `cleanup_running=false` に戻す
...
- URLAdapter が `Settings` インスタンスを受け取る初期化経路を明確にする
