...
def test_settings_priority_dotenv_over_env(tmp_path, monkeypatch, default_settings):
    """.env ファイルが OS 環境変数よりも優先されることを検証する。"""
    from pydantic_settings import SettingsConfigDict

    # default_settings will clear all env vars
...
def test_settings_graph_backend_derivation(
    monkeypatch, default_settings, storage_backend, graph_enabled, expected
):
    """graph_backend は storage_backend + graph_enabled から自動導出される。"""
    # monkeypatch and default_settings will clear all env vars
    ...
    # パラメータ値を優先して設定
    s.storage_backend = storage_backend  # type: ignore[assignment]
    s.graph_enabled = graph_enabled.lower() == "true"
    assert s.graph_backend == expected


def test_settings_embedding_model_derivation(monkeypatch, default_settings):
    ...
    # パラメータ値を優先して設定 (monkeypatch.setenv を生かす)
    s.embedding_provider = "local-model"  # type: ignore[assignment]
    s.local_model_name = "intfloat/multilingual-e5-base"
    ...
    s = Settings(_env_file=None, **default_settings)
    s.embedding_provider = "litellm"  # type: ignore[assignment]
...
def test_settings_dashboard_allowed_hosts_from_env(monkeypatch, default_settings):
    ...
    s2 = Settings(_env_file=None, **default_settings)
...
def test_log_level_validation(default_settings):
    ...
    for level in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
        s = make_settings(**default_settings, log_level=level)
        assert s.log_level == level

    # 無効なログレベル
    with pytest.raises(ValidationError):
        make_settings(**default_settings, log_level="VERBOSE")
    with pytest.raises(ValidationError):
        make_settings(**default_settings, log_level="warn")
...
def test_dashboard_port_validation(default_settings):
    ...
    # 有効なポート
    s = make_settings(**default_settings, dashboard_port=1)
    assert s.dashboard_port == 1
    s = make_settings(**default_settings, dashboard_port=65535)
    assert s.dashboard_port == 65535

    # 範囲外のポート
    with pytest.raises(ValidationError):
        make_settings(**default_settings, dashboard_port=0)
    with pytest.raises(ValidationError):
        make_settings(**default_settings, dashboard_port=65536)
...
def test_dashboard_allowed_hosts_fallback(monkeypatch, default_settings):
    """DASHBOARD_ALLOWED_HOSTS が空や空白のみの場合、デフォルト値にフォールバックする。"""
    # 1. 空文字列
    monkeypatch.setenv("DASHBOARD_ALLOWED_HOSTS", "")
    s1 = Settings(_env_file=None, **default_settings)
    ...
    s3 = Settings(_env_file=None, **default_settings)
