...
| エンドポイント方式 | Fire-and-forget（`"accepted"` を即時返却、処理結果はポーリング不可） | `stdio` の逐次 RPC をブロックしない |
| ジョブ管理 | インメモリ `asyncio.create_task()` + `TaskRegistry`（状態追跡なし） | 外部依存なし、YAGNI |
...
> **Fire-and-forget design:** `"accepted"` はジョブ受理の確認であり、処理完了ではない。
> ステータスのポーリングエンドポイントは存在しない。処理結果（成功・失敗）はサーバーログで確認する。
> `estimated_chunks` はクライアントへの参考値であり、処理後の実際のチャンク数と異なる場合がある。
