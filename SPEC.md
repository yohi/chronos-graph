...
def normalize_rrf(scores: list[float]) -> list[float]:
    ...
    if abs(max_s - min_s) < 1e-8:  # 結果が1件 or 全同スコア
...
3. 検証済みの IP アドレスに対して直接 HTTP リクエストを発行（`Host` ヘッダーは元のホスト名を設定）
4. リダイレクト発生時は、遷移先 URL に対して手順 1-3 を再実行

> **実装注記**: `httpx` のカスタム Transport を使用し、
> DNS 解決と IP 検証を接続前に強制的に実行する。
