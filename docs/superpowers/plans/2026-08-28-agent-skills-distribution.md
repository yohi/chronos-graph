...
Treat installation of `@yohi/opencode-plugin-chronos-turn-end` as an explicit user-owned prerequisite for OpenCode `all` mode. Before returning `SyncPlan`, verify that the user-level Bun configuration source selected by documented precedence maps `@yohi` to `https://npm.pkg.github.com` and has a non-empty credential source for that registry with the GitHub Packages read permission required by the package. Then perform a read-only package-metadata probe against GitHub Packages for `@yohi/opencode-plugin-chronos-turn-end` using the resolved credential; a 401, 403, or equivalent access failure raises `PluginRegistryPrerequisiteError` before any transaction begins. Convert network or credential errors into a redacted typed diagnostic that never exposes or persists the token value, the full response body, or the resolved credential. An environment-referenced token is acceptable only when its environment variable is set. Do not create, rewrite, copy, log, or persist `~/.npmrc`, `bunfig.toml`, token files, or token values; inspect only enough to emit the redacted diagnostic. `package.json` `publishConfig` controls publishing and must not be treated as a consumer-side registry configuration.
...
legacy Save and Recall warnings are read-only; in `selective` mode they do not prevent adding or updating the managed instruction block, so verify only that the legacy prompt bytes and any bytes outside the managed marker block remain unchanged. In `all` mode a legacy Save is rejected before any write or hook setup, and only that rejection path guarantees the entire instruction file remains byte-for-byte unchanged; verify that guarantee in the rejection test and keep the `selective` warning path compatible with managed-block creation or update. legacy Save blocks selective-to-all before hook setup; manual removal allows the next all-mode run
