...
> **【重要な副作用と実装要件】**: 素朴にリクエストURLをIPアドレスに書き換えると、TLSのSNIやHTTPのHostヘッダまでがIPアドレスに切り替わり、証明書検証エラーやバイパスが発生する副作用があります。これを回避するため、`httpcore.AsyncNetworkBackend` をラップしたカスタムバックエンドを実装し、`connect_tcp` メソッドでは検証済みのIPアドレスへルーティングしつつ、`connect_tls` メソッドでは元のホスト名をそのまま渡して厳格な証明書検証を維持する仕組みを構築してください。
