...
- SQLite バックエンドの場合、クリーンアップ処理の最後に WAL 縮小を試行するロジックを実装する：
    1.  まず非同期タスク内で `PRAGMA wal_checkpoint(PASSIVE)` を実行する。
    2.  **自動フェイルセーフの実装**: `PASSIVE` が継続的に失敗（同一ジョブ内で連続3回失敗、または直近10分間で5回以上失敗）し、かつ WAL ファイルの物理サイズが閾値（100MB）を超過していることを検知した場合、システムは警告ログを出力した上で **`PRAGMA wal_checkpoint(TRUNCATE)` を自動的に実行し、自律的なリカバリを試行する**。
    3.  **監視・可視化**: 上記の成否、失敗カウンタの状態、および WAL ファイルサイズの推移をメトリクスとして収集し、運用監視用のデータとして可視化する。
- クリーンアップは `asyncio.create_task()` で非同期実行し、ツール応答をブロックしない。判定用のカウンタやステータスは `lifecycle_state` テーブル等に保持し、プロセス再起動後も継続性を維持すること。
