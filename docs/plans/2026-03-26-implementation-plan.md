...
- 最大トークン制限（`tiktoken` 等を用いた正確なトークン計算を実装）
  ...
  - **近似による過大推定でのガード**: デフォルトエンコーディング（未指定時の `cl100k_base` 等）でのカウントも利用不可能な場合の最終手段として文字数近似を使用する。この際は、**必ず安全側（過大推定）**となる式（例: `ceil(len(text) * safety_margin)` 、safety_margin は 2.0~2.5 等）を採用し、`max_tokens` 超過を防ぐためのガードとする。日本語テキストでは1文字が1〜3トークンとなるため、過大推定となるよう2.0以上の係数を採用すること。
