...
- チャンク化された要素の順序関係を維持するため、メタデータ等にシーケンス情報を含めること
...
- CHUNK_NEXT: URLや手動入力における同一ドキュメント内の連続するチャンク群に対し、時系列の順序関係を構造的に繋ぐエッジを作成すること
...
- SQLite バックエンドの場合、クリーンアップ処理の最後に肥大化した WAL を安全に縮小するため、非同期タスク内で `PRAGMA wal_checkpoint(PASSIVE)` を実行する。ロック競合や後続処理のブロックを避けるため `TRUNCATE` ではなく `PASSIVE` を用いること。
...
- `memory_save` (引数 `source` のデフォルト値はエージェントの自律利用に合わせ `"conversation"` とし、明示的でない場合にも対応するよう実装すること)
- `memory_save_url`
- `memory_search`
- `memory_search_graph`
- `memory_delete`
