...
### Task 1.3b: 構造化ロギングと可観測性 (Observability)
...
git commit -m "feat: 構造化ロギングと可観測性 (Observability) の導入"
```
...
- Create: `tests/integration/test_utils_integration.py`
- Modify: `src/context_store/config.py`
- Modify: `.env.example`

**Step 1: 設定 (Config) の追加とテストの作成**

- `config.py` および `.env.example` に `STALE_LOCK_TIMEOUT_SECONDS`（デフォルト: 600秒/10分）を追加。
...
- `test_stale_lock.py`: `filelock` の取得時、ファイルの `mtime` が有効期限内の場合はスキップ（Timeout）し、`STALE_LOCK_TIMEOUT_SECONDS` を超過している場合はファイルを強制削除して再取得に成功することを検証。テストでは設定値のモック/オーバーライドを行うこと。
- `test_utils_integration.py`: 実ファイルシステムおよび SQLite DB（または temp-file）を用いた結合テストを追加。
  1. **SafeSqliteInterruptCtx**: `SQLiteGraphAdapter` 等の実コンポーネント内で長時間クエリを実行し、本コンテキスト内で意図的に `interrupt()` を発生させる。長時間クエリが適切に中断されること、かつ直後に発行される後続クエリが `OperationalError: interrupted` なしで成功することを検証。
  2. **StaleAwareFileLock**: `LifecycleManager` が利用する想定で、意図的に古い `mtime` を持つロックファイルを作成し、`StaleAwareFileLock` が古いファイルを強制削除して正常にロックを再取得できることを検証。
...
- **Stale Lock 自動リカバリ基盤 (`StaleAwareFileLock`)**:
  LifecycleManager 等のプロセス間排他制御で利用する `filelock` をラップし、プロセス強制終了等によるロックファイルの残留（Stale Lock）を自動修復する機構を実装する。取得時に `config.py` から読み込んだ `STALE_LOCK_TIMEOUT_SECONDS` とファイルの最終更新時刻を比較し、超過していれば強制削除・再取得を行う。

**Step 3: Commit**

```bash
git add src/context_store/utils/ src/context_store/config.py .env.example tests/unit/ tests/integration/
git commit -m "feat: 共通基盤ユーティリティ (SafeSqliteInterruptCtx, StaleAwareFileLock) と設定層・結合テストを実装"
```

**Verification:**

```bash
Run: pytest tests/unit/test_sqlite_interrupt.py tests/unit/test_stale_lock.py tests/integration/test_utils_integration.py -v
Expected: PASS
```
...
- **サーキットブレーカー（クエリタイムアウト）**: 再帰的CTEの実行時（`traverse`等）に、密なグラフでのCPU時間枯渇を防ぐため、明示的なクエリタイムアウト（設定値: `Settings.graph_traversal_timeout_seconds`）を導入すること。
  - **注意**: `aiosqlite` のバックグラウンドスレッドで実行されるクエリは `asyncio.wait_for` では停止できないため、`sqlite3.Connection.interrupt()` を用いて強制終了させる必要があります。
  - **【重要な副作用と実装要件】**: 非同期タスクから単純に `interrupt()` を呼び出すと、クエリ完了後のアイドル状態のコネクションに割り込みフラグが残り、**後続の無関係なクエリが `OperationalError: interrupted` でクラッシュする**という深刻な副作用があります。これを防ぐため、実行中状態を管理するフラグやロックを用いたラッパークラスを実装し、**「クエリが確実に実行中である期間のみ `interrupt()` を発火させる」**厳密な状態管理を行ってください。
  - タイムアウト発生時は例外を送出せず、到達済みの部分グラフを返すか、安全に空結果を返す Graceful Degradation を実装し、到達したノード数を含む `graph_traversal_timeout` イベントの警告ログを出力すること。
...
6. コネクション確立後、レスポンスボディを `httpx` のストリーミングAPI (`stream`) を用いて少しずつ読み込み、データ量が 10MB を超過した時点で直ちに通信を中断する。この際、中途半端な状態で切断されたソケットがコネクションプールに返却され、後続のリクエストが破損する（プール汚染）のを防ぐため、必ず `httpx.Response.aclose()` を呼び出すか、コネクションの強制破棄（ストリームの完全なクローズ）を保証すること。
...
- **コードブロック保護**: 分割境界がコードブロック（```` ``` ````）の内部に該当する場合、コードブロックの途中でチャンクが分断されないこと
...
また、Markdown文書の分割処理において、分割境界がコードブロックの内部に該当する場合は、コードブロックの終了まで分割を遅延させる、または意味的ブロックを維持するスマートチャンキングロジックを実装すること。
...
**排他制御 (Race Condition対策):** 同一コンテンツまたはURLの並行処理による重複登録（TOCTOU脆弱性）を防ぐため、Ingestion Pipeline レベルでコンテンツハッシュやURL等に基づく一時的な非同期ロック（メモリ上のハッシュベースの `asyncio.Lock` 機構等）を導入し、Deduplicator（検索・重複チェック）から保存完了までの区間を保護してください。ただし、この排他制御は同一プロセス内でのみ有効なベストエフォートの防衛であり、すり抜けた重複は Task 6.3 の Consolidator が結果整合性で修復します。永続化層の防衛ラインとして、データベースレベルでの一意制約（`content_hash` 列に対する `UNIQUE` インデックス等）も実装しますが、プロセス間をまたぐ意味的類似度の重複修復はConsolidatorに委ねる設計としてください。
...
- **テスト要件（自己修復機能）**:
  - **重複検出ロジックのテスト**: `Consolidator` が最近保存されたN件に対して類似度検索を行い、≥ 0.90 のペアを正しく検出することを検証。**O(N)のフルスキャンを避けていること（例: モックでの呼び出し回数やクエリ構造の検証）をテスト要件に含めること。**
  - **レースコンディションのシミュレーション**: 複数の並行 `memory_save` 呼び出しで `Deduplicator` が見逃したケースを再現し、`Consolidator` が事後修復することを検証。
  - **優先順位テスト**: `Consolidator` が 0.85–0.89 の通常候補と ≥0.90 の自己修復候補を混在処理する際に、自己修復を優先して処理することを検証。
  - **パフォーマンステスト**: ≥10,000件のモックデータセット環境で、`Consolidator` の自己修復処理が一定時間（例: 30秒）以内に完了することを要件化。
  - **監視ログのテスト**: 自己修復発動時に INFO/WARNING レベルで `'Self-healing: archived duplicate memory {id} due to similarity {score}'` 相当のログが出力されることを検証。
- **実装手順**:
  - 統合候補（Deduplicator がマーク済み、または類似度 ≥ 0.90 だが重複登録されてしまった記憶）の記憶群を取得。**自己修復候補の取得時は、HNSWインデックスを活用したバッチ絞り込み（スライディングウィンドウ方式）を行い、O(N)のフルスキャンを避けること。**
    - **スライディングウィンドウの定義と基準**: 「直近更新分」の基準として「前回クリーンアップ実行時（`last_cleanup_at`）以降に作成・更新された記憶ノード」を動的ウィンドウとして採用する（推奨）。
    - **ウィンドウサイズとバッチ処理**: メモリ枯渇と処理時間の長大化を防ぐため、1回のクリーンアップサイクルで処理する自己修復対象ノード数に上限（例: トップK=`CONSOLIDATION_BATCH_SIZE`、デフォルト100〜500件）を設ける。
    - **HNSW検索のパラメータとクエリ想定**: 抽出した各候補ノードに対して、HNSWインデックスを用いた近似近傍探索を発行する。具体的には `StorageAdapter.vector_search(embedding, top_k=5)` 等のクエリ経路（または基盤となる SQLite `MATCH` / PostgreSQL `ORDER BY ... <=>` の直接発行）を利用し、上位結果の中から類似度閾値（≥ 0.90）を超えるものをメモリ上でフィルタリングする。これにより、全件同士の総当たり比較（O(N^2)）を O(M log N)（Mはウィンドウ内の候補数）に抑える。
  - 最新の記憶をベースに、古い記憶の情報をマージ（重複の場合は事後的な Append-only 置換として古い方をアーカイブ）
...
- **OSレベルの排他制御**: `filelock` を用いたOSレベルの排他ロック（`.lifecycle.lock` 等）の取得と解放処理を実装する。マルチプロセス環境での同時実行を防ぐため、ロック取得失敗時（`Timeout` 等）は直ちにスキップしてDBアクセスを行わない（サイレントリターンする）ロジックとし、その挙動をテストで確認すること。
