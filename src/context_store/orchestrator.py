    ...
    async def _check_vector_dimension(self) -> None:
        ...
        if stored_dim != current_dim:
            raise ConfigurationError(
                ...
                "リカバリ手順:\n"
                "1. モデルを変更した場合: 'uv run python scripts/migrate_dimension.py' "
                "を実行して再埋め込みを行ってください。\n"
                "2. スキーマが古い場合 (PostgreSQL/Supabase): docs/migration.md の該当する"
                "移行手順に従ってください。既存の embedding を embedding_old に保持し、"
                "新しい次元のカラムへ再埋め込みし、全件検証後に切り替えてください。\n"
                "3. 設定ミスの場合: .env の EMBEDDING_DIMENSION をストレージと一致するように"
                "修正してください。\n"
                "4. 別環境として開始する場合:\n"
                "   SQLite: SQLITE_DB_PATH 環境変数を新しいパスに変更してください。\n"
                "   PostgreSQL: 別の DB 名（POSTGRES_DB）を指定してください。\n"
                "5. 全データを初期化する場合:\n"
                "   SQLite: DB ファイルを手動で削除してください。\n"
                "   PostgreSQL: スキーマを再構築してください（DROP SCHEMA public CASCADE など）。"
            )
