...
- SQLite バックエンドの場合、クリーンアップ処理の最後に肥大化した WAL を安全に縮小するため、非同期タスク内で `PRAGMA wal_checkpoint(PASSIVE)` を実行する。ロック競合や後続処理のブロックを避けるため `TRUNCATE` ではなく `PASSIVE` を用いること。さらに、実行結果（ステータスコードや "busy" 状態）と WAL ファイルサイズの推移（現在サイズ／時間系列）をメトリクスとして収集・可視化し、アラートやダッシュボードに組み込む運用監視要件を実装すること。
