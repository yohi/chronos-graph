...
| `tags` | text[] | プロジェクトタグ等 |
| `project` | text? | プロジェクト識別子。MCP ツール受け口で自動正規化される：ローカルに存在するパスは git リポジトリルートのディレクトリ名に解決し、存在しない場合は basename を採用して前後の空白を除去、末尾の `/` を取り除き、小文字化する。例: `/home/user/repo` → `repo`、`/path/to/chronos-graph` → `chronos-graph`、`  DotFiles-AI/ ` → `dotfiles-ai`。
