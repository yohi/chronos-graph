...
- **実装手順**:
  - 統合候補（Deduplicator がマーク済み、または類似度 ≥ 0.90 だが重複登録されてしまった記憶）の記憶群を取得。**自己修復候補の取得時は、HNSWインデックスを活用したバッチ絞り込み（直近更新分のみを対象とするスライディングウィンドウ方式等）を行い、O(N)のフルスキャンを避けること。**
...
ization し、同一 10,000 件 fixture を使って決定論的に実行する
- ベンチ結果は Task 9.4 の成果物として保存し、SPEC.md §13 の目標値との比較を記録する
