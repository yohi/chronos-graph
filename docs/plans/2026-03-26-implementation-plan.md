...
- **WAL設定の外部化と設定値の利用（config.py / .env）:**
    ...
    - 環境変数から読み込む `Settings` オブジェクトに以下を追加し `.env.example` にも記載する：
        - `WAL_THRESHOLD_BYTES` (例: 104857600)
        - `WAL_CONSECUTIVE_PASSIVE_FAILURES` (例: 3)
        - `WAL_FAILURE_WINDOW_SECONDS` (例: 600)
        - `WAL_FAILURE_WINDOW_COUNT` (例: 5)
...
- **WALステータスの永続化スキーマと更新ルール:**
    - 判定用のカウンタやステータスは `lifecycle_state` テーブル等に漠然と保持するのではなく、`lifecycle_state` テーブル（または新規の `lifecycle_wal_state` テーブル）に以下の明確な名前と型のカラムを追加し永続化する：
        ...
        - `wal_failure_window` (JSON): 過去設定時間（例: `WAL_FAILURE_WINDOW_SECONDS`）の失敗タイムスタンプの配列（リングバッファ）を保持するスライディングウィンドウ
    - **非同期チェックポイントタスクによる更新ルール:**
        ...
        - スライディングウィンドウの更新：状態更新のたびに `wal_failure_window` から現在時刻より設定秒数（例: `WAL_FAILURE_WINDOW_SECONDS`）以上古いエントリをパージする。このフィールドをSource of Truthとして設定時間内の失敗回数を計算し、これを用いて「設定時間に設定回数（例: 5回）以上の失敗」などの閾値判定を行うこと。
