...
**排他制御 (Race Condition対策):** 同一コンテンツまたはURLの並行処理による重複登録（TOCTOU脆弱性）を防ぐため、Ingestion Pipeline レベルでコンテンツハッシュやURL等に基づく一時的な非同期ロック（メモリ上のハッシュベースの `asyncio.Lock` 機構等）を導入し、Deduplicator（検索・重複チェック）から保存完了までの区間を保護してください。
