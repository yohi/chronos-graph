...
- SQLite バックエンドの場合、クリーンアップ処理の最後に WAL 縮小を試行するロジックを実装する：
    ...
    2.  **自動フォールバック機構の実装**: 「`PASSIVE` が継続的に失敗」かつ「WALファイルサイズが閾値（例: 100MB）を超過」という**両条件を満たした場合に警告ログ出力 + 自動的に `PRAGMA wal_checkpoint(TRUNCATE)` を試行**するシステムによる即座の自動リカバリ（自動フェイルセーフ）ロジックを実装する。
    3.  **監視・可視化**: 上記とは独立して、手動介入や運用監視を目的とし、実行結果の成否、失敗カウンタの状態、および WAL ファイルサイズの推移をメトリクスとして収集し可視化する。
