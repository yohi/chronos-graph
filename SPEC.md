...
| 条件 | アクション |
|---|---|
| 同一プロジェクト & コサイン類似度 ≥ 0.90 | 既存記憶を上書き更新 |
...
| `SUPERSEDES` | Deduplicator が上書き更新を実行 | ✅ |
...
```text
rrf_score = Σ (weight × 1/(K + rank + 1))    # K = 60
...
| ジョブ | 頻度 | 処理 |
|---|---|---|
| Decay Scorer | 毎日 | 全 Active 記憶の複合スコア再計算。閾値以下をアーカイブ候補にマーク |
| Auto Archiver | 毎日 | マークされた記憶を Archived に遷移。Neo4j ノードに `archived` フラグ付与 |
| Consolidator | 週 1 回 | Deduplicator がマークした統合候補をマージ |
| Purger | 週 1 回 | Archived 後 N 日経過した記憶を物理削除（PostgreSQL + Neo4j 連動） |
| Stats Collector | 毎日 | DB 使用量・記憶数・平均スコア等の統計記録 |
