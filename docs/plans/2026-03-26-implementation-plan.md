...
5. コネクション確立後、まずレスポンスヘッダーの `Content-Type` が許可リストに含まれることを検証し（ボディのストリーミング開始前・ヘッダー受信直後に行うこと）、その後レスポンスボディを `httpx` のストリーミングAPI (`stream`) を用いて少しずつ読み込み、データ量が 10MB を超過した時点で直ちに通信を中断する。
